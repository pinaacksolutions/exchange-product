...
{
    public abstract class SampleBarterAppTestBase : AbpIntegratedTestBase<SampleBarterAppTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SampleBarterAppTestBase()
        {
        ...
        protected void UsingDbContext(Action<SampleBarterAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SampleBarterAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SampleBarterAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SampleBarterAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SampleBarterAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SampleBarterAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SampleBarterAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SampleBarterAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SampleBarterAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SampleBarterAppDbContext, Task<T>> func)
        {
            T result;
